  ...
  return (
    <SidebarLayout title="Mark Attendance" breadcrumbs="Attendance / Mark Roster">
      ...
      <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 class="text-2xl font-black text-slate-900 tracking-tight">Daily Roll Call</h2>
        ...
        <button
          ...
          class="px-6 py-3 rounded-xl bg-emerald-500 hover:bg-emerald-600 text-white font-extrabold text-xs shadow-md shadow-emerald-500/20 flex items-center gap-2 transition disabled:opacity-50 shrink-0"
        >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
              {TEAMS.map((team) => (
                <button
                  ...
                  class={`py-2.5 px-3 rounded-xl text-xs font-bold transition-all border text-center ${
                    selectedTeam === team
          ...
          <div class="space-y-2">
            ...
            <input
              ...
              class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-900 focus:outline-none focus:border-emerald-500 transition"
            />
          </div>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div class="flex items-center gap-4 text-xs font-bold flex-wrap">
            <div class="flex items-center gap-1.5 text-slate-600">
              <Users class="w-4 h-4 text-emerald-600" /> Roster: <span class="text-slate-900">{roster.length}</span>
            </div>
            <div class="flex items-center gap-1.5 text-emerald-700">
              <CheckCircle2 class="w-4 h-4" /> Present (P): <span>{presentCount}</span>
            </div>
            <div class="flex items-center gap-1.5 text-rose-700">
              <XCircle class="w-4 h-4" /> Absent (A): <span>{absentCount}</span>
            </div>
            <div class="flex items-center gap-1.5 text-amber-700">
              <Calendar class="w-4 h-4" /> Leave (L): <span>{leaveCount}</span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 w-full sm:w-auto flex-wrap">
            <button
              ...
              class="flex-1 sm:flex-initial px-3.5 py-2 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold transition flex items-center justify-center gap-1.5"
            >
              <CheckCircle2 class="w-3.5 h-3.5" /> All Present (P)
            </button>
            <button
              type="button"
              onClick={() => handleMarkAll('Absent')}
              class="flex-1 sm:flex-initial px-3.5 py-2 rounded-xl bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200 text-xs font-bold transition flex items-center justify-center gap-1.5"
            >
              <XCircle class="w-3.5 h-3.5" /> All Absent (A)
            </button>
            <button
              type="button"
              onClick={() => handleMarkAll('Leave')}
              class="flex-1 sm:flex-initial px-3.5 py-2 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 text-xs font-bold transition flex items-center justify-center gap-1.5"
            >
              <Calendar class="w-3.5 h-3.5" /> All Leave (L)
            </button>
          </div>
        </div>
      </div>
      ...
      <div class="relative max-w-md">
        ...
        <input
          ...
          class="w-full pl-10 pr-4 py-2.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 transition shadow-xs"
        />
      </div>
      ...
      {loading ? (
        <div class="h-64 flex flex-col items-center justify-center text-slate-400 space-y-3">
          ...
          <p class="text-xs font-semibold">Loading roster for {selectedTeam}...</p>
        </div>
      ) : filteredRoster.length === 0 ? (
        <div class="bg-white border border-slate-200/80 rounded-2xl p-12 text-center text-slate-500 shadow-xs">
          <h3 class="text-base font-bold text-slate-900">No active students found</h3>
          <p class="text-xs mt-1">There are no active students matching your query.</p>
        </div>
      ) : (
            ...
            <table class="w-full text-left border-collapse">
              ...
              <tbody class="divide-y divide-slate-100 text-sm">
                {filteredRoster.map((student, idx) => (
                  <tr key={student.studentId} class="hover:bg-slate-50/80 transition">
                    ...
                    <td class="py-4 px-6">
                      <div class="flex items-center gap-2">
                        ...
                        {student.status === 'Present' && (
                          <span class="px-2 py-0.5 rounded text-[10px] font-black bg-emerald-500 text-white shadow-xs">P</span>
                        )}
                        {student.status === 'Absent' && (
                          <span class="px-2 py-0.5 rounded text-[10px] font-black bg-rose-500 text-white shadow-xs">A</span>
                        )}
                        {student.status === 'Leave' && (
                          <span class="px-2 py-0.5 rounded text-[10px] font-black bg-amber-500 text-white shadow-xs">L</span>
                        )}
                      </div>
                    ...
                    <td class="py-4 px-6">
                      <div class="flex items-center justify-center gap-2">
                        <button
                          ...
                          class={`px-3 py-1.5 rounded-xl text-xs font-bold transition shadow-xs flex items-center gap-1 ${
                            student.status === 'Present'
                              ? 'bg-emerald-500 text-white shadow-emerald-500/20 ring-2 ring-emerald-400'
                              : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                          }`}
                        >
                          <span class="font-black text-[10px]">P</span> Present
                        </button>
                        ...
                        >
                          <span class="font-black text-[10px]">A</span> Absent
                        </button>
                        ...
                        >
                          <span class="font-black text-[10px]">L</span> Leave
                        </button>
                      </div>
                    </td>
                  </tr>
